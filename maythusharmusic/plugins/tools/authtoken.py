...
async def check_auth_token():
    video_url = "https://youtu.be/pRfmrE0ToTo?si=gk4rOPEWphYIVaGH"
...
async def cookies_status(client, message):
    ...
    cookie_status = await check_cookies("https://youtu.be/pRfmrE0ToTo?si=gk4rOPEWphYIVaGH")
    status_message = "**Cookie Status:**\n"
...
async def auth_token_status(client, message):
    ...
    if not use_token:
        ...
        try:
            os.system(
                f"yt-dlp --username oauth2 --password '' -F https://youtu.be/pRfmrE0ToTo?si=gk4rOPEWphYIVaGH"
            )
